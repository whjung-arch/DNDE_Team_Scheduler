...
- items: 견적 품목 배열 (표 형태라면 해당 행의 프로젝트, 용량, 규격, 품명 등 모든 텍스트를 종합해 매우 구체적으로 기재. 단일 용역명 형태라면 그 내용을 상세히 기재)
- qty: 수량 (숫자, 없으면 1)
- unitPrice: 단가 (숫자, 없으면 총액과 동일)
- amount: 금액 (숫자)
- supplyPrice: 공급가액 (숫자)
- vat: 부가세 (숫자)
- totalAmount: 총금액 (숫자)
- assignee: 발신(우리 회사) 담당자 이름 (수신처 아님)
...
- contractDate: 계약(발주) 시작일자 (YYYY-MM-DD 형식)
- items: 품목 배열 (가장 중요함. 표 형태라면 해당 행의 모든 텍스트(프로젝트, 용량, 규격, 품명, 품번 등)를 하나로 길게 합쳐서 아주 구체적인 이름으로 기재할 것. 단일 용역명 형태라면 그 내용을 상세히 기재)
- qty: 수량 (숫자, 없으면 1)
- unitPrice: 단가 (숫자, 없으면 총액과 동일)
- amount: 금액 (숫자)
- supplyPrice: 공급가액 (숫자)
- vat: 부가세 (숫자)
- totalAmount: 총계약(발주)금액 (숫자)
- assignee: 발신(우리 회사) 담당자 이름
