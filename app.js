...
항목:
{
  ...
  "items": [{"name": "품목명 (품명, 규격, 프로젝트명 등이 표에 나뉘어 있다면 문맥을 파악해 하나의 구체적인 이름으로 합쳐서 작성)", "qty": 수량(숫자), "unitPrice": 단가(숫자), "amount": 금액(숫자)}],
  ...
  "assignee": "견적서를 발송/작성한 우리 회사(발신자) 담당자 이름 (수신처 담당자가 아님. 영업사원 등 핵심 이름만 추출. 못찾으면 빈문자열)"
}
...
특히 계약일(Contract Date)과 계약기간(Contract Period)을 정확히 분석해. 발주서의 경우 납기일(Promised Date, 요구납기일, 납품기한 등)이 계약기간(Period)에 해당하므로 이를 참고해.
...
항목:
{
  ...
  "items": [{"name": "품목명 (품명, 규격, 프로젝트명 등이 표에 나뉘어 있다면 문맥을 파악해 하나의 구체적인 이름으로 합쳐서 작성. 예: '[정부과제] 3MVA급 변압기 사외외주')", "qty": 수량(숫자), "unitPrice": 단가(숫자), "amount": 금액(숫자)}],
  "supplyPrice": 공급가액(원 기호나 쉼표를 제외한 순수 숫자만. 표에 없으면 계산해서라도 넣을 것),
  "vat": 부가세(원 기호나 쉼표를 제외한 순수 숫자만. 표에 없으면 계산해서라도 넣을 것),
  "totalAmount": 총계약금액(원 기호나 쉼표를 제외한 순수 숫자만. '총금액' 칸이 없고 '금액'만 있다면 그걸 총계약금액으로 사용),
  "assignee": "계약서를 발송/작성한 우리 회사(발신자) 담당자 이름 (수신처 담당자가 아님. 영업사원 등 핵심 이름만 추출. 못찾으면 빈문자열)",
  "contractPeriod": "계약기간 또는 납기일 (예: 2026.01.01 ~ 2026.12.31). 특히 promised date, 요구납기일, 납품기한을 참고하여 정확하게 판단할 것. 찾을수 없으면 빈문자열"
}
